...
if(!empty($_POST)) {
    if(isset($_POST['add'])) {
        $sql = "INSERT INTO title_to_genre(id_title, id_genre) VALUES (:id_title, :id_genre)";
        $queryAdd = $pdo->prepare($sql);
        $value = array(':id_title'=>$_POST['id_title'], ':id_genre'=>$_POST['id_genre']);
        ...
        echo "<a class='m-3 btn btn-outline-dark' href='TTitleToGenre.php'>Назад</a>";
    }

    if(isset($_POST['delete'])) {
        //print_r($_POST);
        $sql = "DELETE FROM title_to_genre WHERE id=:id";
    ...
    if(isset($_POST['edit'])) {
        $sql = "UPDATE title_to_genre SET id_title = :id_title, id_genre = :id_genre WHERE id=:id";
        $queryEdit= $pdo->prepare($sql);
        $value = array(':id_title'=>$_POST['id_title'], ':id_genre'=>$_POST['id_genre'], ':id'=>$_POST['selected']);
